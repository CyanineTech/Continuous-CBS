...
struct Node {
  ...
  int interval_id;
  Node(int _id = -1, double _f = -1, double _g = -1, double _i = -1,
       double _j = -1, Node* _parent = nullptr, double begin = -1,
       double end = -1)
      : id(_id),
        f(_f),
        g(_g),
...
struct Path {
  ...
  int agentID;
  int expanded;
  Path(std::vector<Node> _nodes = std::vector<Node>(0), double _cost = -1,
       int _agentID = -1)
...
struct sPath {
  ...
  int agentID;
  int expanded;
  sPath(std::vector<sNode> _nodes = std::vector<sNode>(0), double _cost = -1,
        int _agentID = -1)
      : nodes(_nodes), cost(_cost), agentID(_agentID) {
    expanded = 0;
  }
  sPath operator=(const Path& path) {
    ...
    nodes.clear();
    for (auto n : path.nodes) nodes.push_back(sNode(n));
    return *this;
  }
};
...
struct Constraint {
  int agent;
  double t1, t2;  // prohibited to start moving from (i1, j1) to (i2, j2) during
                  // interval (t1, t2)
  // double i1, j1, i2, j2; // in case of node constraint i1==i2, j1==j2.
  int id1, id2;
  bool positive;
  Constraint(int _agent = -1, double _t1 = -1, double _t2 = -1, int _id1 = -1,
             int _id2 = -1, bool _positive = false)
      : agent(_agent),
        t1(_t1),
        t2(_t2),
        id1(_id1),
  ...
  bool operator<(const Constraint& other) const { return t1 < other.t1; }
};
...
struct Conflict {
  ...
  double overcost;
  // sPath path1, path2;
  Conflict(int _agent1 = -1, int _agent2 = -1, Move _move1 = Move(),
           Move _move2 = Move(), double _t = CN_INFINITY)
...
struct CBS_Node {
  ...
  CBS_Node(std::vector<sPath> _paths = {}, CBS_Node* _parent = nullptr,
           Constraint _constraint = Constraint(), double _cost = 0,
           int _conflicts_num = 0, int total_cons_ = 0)
      : paths(_paths),
        parent(_parent),
        constraint(_constraint),
        cost(_cost),
        conflicts_num(_conflicts_num),
        total_cons(total_cons_) {
...
struct Open_Elem {
  ...
  Open_Elem(CBS_Node* _tree_pointer = nullptr, int _id = -1, double _cost = -1,
            unsigned int _cons_num = 0, unsigned int _conflicts_num = 0)
      : tree_pointer(_tree_pointer),
        id(_id),
        cost(_cost),
        cons_num(_cons_num),
        conflicts_num(_conflicts_num) {}
  ~Open_Elem() { tree_pointer = nullptr; }
};
...
typedef multi_index_container<
    Open_Elem,
    indexed_by<
        // ordered_non_unique<tag<cost>, BOOST_MULTI_INDEX_MEMBER(Open_Elem,
        // double, cost)>,
        ordered_non_unique<
            composite_key<
                Open_Elem, BOOST_MULTI_INDEX_MEMBER(Open_Elem, double, cost),
                BOOST_MULTI_INDEX_MEMBER(Open_Elem, unsigned int,
                                         conflicts_num),
                BOOST_MULTI_INDEX_MEMBER(Open_Elem, unsigned int, cons_num)>,
            composite_key_compare<std::less<double>, std::less<int>,
                                  std::greater<int>>>,
...
struct Focal_Elem {
  ...
  double cost;
  Focal_Elem(int id_ = -1, unsigned int conflicts_num_ = 0,
             unsigned int constraints_ = 0, double cost_ = 0)
      : id(id_),
        conflicts_num(conflicts_num_),
...
typedef multi_index_container<
    ...
    indexed_by<
        ...
        // ordered_non_unique<tag<constraints_num>,
        // BOOST_MULTI_INDEX_MEMBER(Focal_Elem, int, constraints)>,
        hashed_unique<tag<id>, BOOST_MULTI_INDEX_MEMBER(Focal_Elem, int, id)>>>
    Focal_container;

typedef multi_index_container<
    Node, indexed_by<
              // ordered_non_unique<tag<cost>,
...
class CBS_Tree {
 ...
 public:
  ...
  // 新增：获取 FOCAL 列表的大小
  size_t get_focal_size() const { return focal.size(); }

  // 新增：获取 OPEN 列表中最好节点的统计信息 {成本, 冲突数}
  std::pair<double, unsigned int> get_open_best_stats() const {
    if (container.empty()) {
      return {-1.0, 0};
    }
    const auto& best = *container.get<0>().begin();
    return {best.cost, best.conflicts_num};
  }

  // 新增：获取 FOCAL 列表中最好节点的统计信息 {冲突数, 成本}
  std::pair<unsigned int, double> get_focal_best_stats() const {
    if (focal.empty()) {
      return {0, -1.0};
    }
    const auto& best = *focal.get<0>().begin();
    return {best.conflicts_num, best.cost};
  }

  void add_node(CBS_Node node) {
    tree.push_back(node);
    container.insert(Open_Elem(&tree.back(), node.id, node.cost,
                               node.total_cons, node.conflicts_num));
    open_size++;
    if (focal_weight > 1.0)
      if (container.get<0>().begin()->cost * focal_weight > node.cost)
        focal.insert(Focal_Elem(node.id, node.conflicts_num, node.total_cons,
                                node.cost));
  }
  ...
  void update_focal(double cost) {
    ...
    for (auto it = it0; it != it1; it++)
      focal.insert(
          Focal_Elem(it->id, it->conflicts_num, it->cons_num, it->cost));
  }
  ...
  std::vector<sPath> get_paths(CBS_Node node, int size) {
    ...
    for (unsigned int i = 0; i < node.paths.size(); i++)
      if (paths.at(i).nodes.empty()) paths.at(i) = node.paths.at(i);
    return paths;
  }
};
...
struct Solution {
  ...
  std::vector<sPath> paths;
  Solution(double _flowtime = -1, double _makespan = -1,
           std::vector<sPath> _paths = {})
...
class Vector2D {
 public:
  ...
  inline Vector2D operator+(const Vector2D& vec) {
    return Vector2D(i + vec.i, j + vec.j);
  }
  inline Vector2D operator-(const Vector2D& vec) {
    return Vector2D(i - vec.i, j - vec.j);
  }
...
class Point {
 public:
  ...
  int classify(Point& pO, Point& p1) {
    ...
    if (sa > 0.0) return 1;                                           // LEFT;
    if (sa < 0.0) return 2;                                           // RIGHT;
    if ((a.i * b.i < 0.0) || (a.j * b.j < 0.0)) return 3;             // BEHIND;
    if ((a.i * a.i + a.j * a.j) < (b.i * b.i + b.j * b.j)) return 4;  // BEYOND;
    if (pO == p2) return 5;                                           // ORIGIN;
    if (p1 == p2) return 6;  // DESTINATION;
    return 7;                // BETWEEN;
  }
};
